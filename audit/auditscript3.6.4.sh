...
scriptn="Linux auditscript 3.6.4"
...
echo "Script message ***** checking for virtualbox *****"

# Check if VirtualBox is installed and get the version
VB_VERSION=$(VBoxManage --version 2>/dev/null)

if [ $? -eq 0 ]; then
    echo "VirtualBox version: $VB_VERSION"
    echo "VirtualBox version: $VB_VERSION" >> "$output_file"
else
    echo "VirtualBox is not installed on this system." >> "$output_file"
...
VBoxManage list extpacks >> "$output_file"

# List all VirtualBox VMs
echo "Listing all VirtualBox VMs:"
VBoxManage list vms >> "$output_file"
VBoxManage list vms

# Get detailed information for all VMs
echo "Getting detailed information for all VMs:"
VBoxManage list vms | cut -d '"' -f 2 | while read -r VM_NAME; do
    echo "Details for VM: $VM_NAME"
    VBoxManage showvminfo "$VM_NAME" --machinereadable
    VBoxManage showvminfo "$VM_NAME" --machinereadable >> "$output_file"
